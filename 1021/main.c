...
int getValueInCents(double value)
{
...
int main()
{
  ...
  scanf("%lf", &amount);
  int amountInCents = getValueInCents(amount);
  for (int valueIndex = 0; valueIndex < VALUES_LENGTH; valueIndex++)
  {

    double quantityOf = (int)(amountInCents / getValueInCents(values[valueIndex][0]));

    if (quantityOf > 0)
    {
      amountInCents = amountInCents % getValueInCents(values[valueIndex][0]);
    }
